...
public class BasicStatisticsTestDrive {
    ...
    public static void testMyArrayList() {
        ...
        MyList dataList = (MyList) new MyArrayList();
        for (int i = 0; i < length; i++) {
            double randomValue = random.nextDouble(1,20);
            dataList.append(randomValue);
        }

        // Sử dụng BasicStatistic để tính các đại lượng thống kê cơ bản
        BasicStatistic basicStatistic = new BasicStatistic((mylist.MyList) dataList);

        // In ra dữ liệu và các đại lượng thống kê
        System.out.println("Dữ liệu:");
        System.out.println(dataList);
        System.out.println("Max: " + basicStatistic.max());
        System.out.println("Min: " + basicStatistic.min());
        System.out.println("Kỳ vọng: " + basicStatistic.mean());
        System.out.println("Phương sai: " + basicStatistic.variance());
    }
